        ...
        for (var i = 0; i < sjzEvt.length ; i++){
            ...
            for(var key in map){
                ...
                if(map[key].tagType == 'text_datetime'){
                    val_ = (typeof(val_) == 'undefined' || val_ == null) ?"":new Date(val_).Format(map[key].dateFormat);
                    td_tag = $(td_tag).text(val_);
                }
                // 分类文本类型
...
function get_search_criteria(currentPage,pageSize){
    ...
    for(var i = 0 ; i < criTags.length ; i++ ){
        var jTag = $(criTags[i]);
        if(jTag.attr("type") == 'radio'){
            if(jTag.attr("checked") == "checked"){
