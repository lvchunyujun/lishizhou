...
public interface InnerMailMapper
{
	 ...
	 * @return
	 */
	//@Select("SELECT *,(CASE TO_DAYS(now())-TO_DAYS(sendtime) WHEN 0 THEN 0 WHEN 1 THEN 1 ELSE -1 END) timeOrder FROM inner_mail WHERE receiver = #{userId} ORDER BY sendtime DESC LIMIT #{offset}, #{pagesize}")
